...
└── issues/                            # 품질 관리 및 이슈 추적
    ...
    └── usability_review.md            # 🧭 실사용 점검 — 🛡️ 안전장치(S) · 쓰는 사람이 막히는 것(U-1~U-18)
```
...
* **[usability_review.md](issues/usability_review.md)**: 🧭 **실사용 점검 (2026-09-24).** 두 묶음입니다. **🛡️ 안전장치(S)** 는 앱이 꺼지거나 잘못 눌러도 작업이 남게 하는 것 — 자동 저장 · 전역 예외 처리 · 로그 위치 · `.bak` 세대 · 일괄 작업 확인창 · 종료 크래시 · 밖에서 고친 파일 덮어쓰기 — 으로 **가장 먼저** 할 일입니다. **🧭 사용성(U)** 은 쓰는 사람이 매일 막히는 것 18건을 🥇 → 🥉 순으로 정리했습니다. 헤드리스 창과 실제 파서로 재현한 두 건(`Space` 가 방금 누른 버튼으로 감 · 드라이브 맨 위 차트를 못 엶)을 포함하고, 항목마다 **지금 / 왜 문제인가 / 고칠 방향**을 코드 위치와 함께 적었습니다.
